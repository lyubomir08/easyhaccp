...
const isOutOfNorm = (temp, norm_min, norm_max) => {
    const lo = Math.min(norm_min ?? -2, norm_max ?? 4);
    const hi = Math.max(norm_min ?? -2, norm_max ?? 4);
        ...
        <div className="max-w-6xl mx-auto space-y-8 p-4">
            ...
            {form.object_id && (
                <form onSubmit={onSubmit} className="bg-white border rounded-xl p-6 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1">Хладилник <span className="text-red-500">*</span></label>
                            <select name="fridge_id" value={form.fridge_id} onChange={onChange} required className="border px-3 py-2 rounded-md w-full">
                        ...
                        <div>
                            ...
                            <input disabled value={selectedFridge ? `${selectedFridge.norm_min}°C / ${selectedFridge.norm_max}°C` : "−2°C / +4°C"} className="border px-3 py-2 rounded-md w-full bg-slate-100 text-slate-700 font-medium" />
                            {selectedFridge && <p className="text-xs text-gray-500 mt-1">Норма за избрания хладилник</p>}
                        </div>
                        <div>
                            <label className="block text-sm font-medium mb-1">Измерена температура (°C) <span className="text-red-500">*</span></label>
                            <input type="number" step="0.1" name="measured_temp" value={form.measured_temp} onChange={onChange} placeholder="Стойност °C" required className="border px-3 py-2 rounded-md w-full" />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input name="corrective_action" value={form.corrective_action} onChange={onChange} placeholder="Мерки при отклонение" className="border px-3 py-2 rounded-md w-full" />
                        </div>
                    </div>
            ...
            {form.object_id && (
                <div>
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Търси по хладилник..." className="border px-3 py-2 rounded-md w-full" />
                </div>
            )}
                            ...
                            <div key={l._id} className="bg-white border rounded-xl p-5 flex justify-between items-start">
                                <div className="space-y-1 flex-1">
                                    ...
                                    <div className="text-sm text-slate-600">
                                        Норма: <span className="font-medium text-slate-800">{l.fridge_id?.norm_min}°C / {l.fridge_id?.norm_max}°C</span>
                                    </div>
                                    <div className="text-sm text-slate-600">
                                        Температура:{" "}
                                        <span className={outOfNorm ? "text-red-600 font-semibold" : "text-green-600 font-medium"}>
                                            {l.measured_temp} °C
                                        </span>
                                        {outOfNorm && <span className="ml-2 text-red-600 text-xs">⚠ Извън норма</span>}
                                    </div>
                                    <div className="text-sm text-slate-600">
                                        Служител: <span className="font-medium text-slate-800">{l.employee_id ? `${l.employee_id.first_name} ${l.employee_id.last_name}` : "Не е посочен"}</span>
                                    </div>
                                    {l.corrective_action && (
                                        <div className="text-sm text-slate-600">Мерки: <span className="font-medium text-slate-800">{l.corrective_action}</span></div>
                                    )}
